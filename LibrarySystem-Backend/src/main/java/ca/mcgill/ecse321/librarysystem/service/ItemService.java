...
public class ItemService {
	 ...
	 * @return throws IllegalArgumentException if input is incorrect or if the item
	 *         is borrowed or in the archives.
	 * 
	 * @author Sami
	 */
	...
	public Item borrowItem(int itemId, String itemName, int patronId) {
		...
		if (patronOfInterest.getBorrowedAlbums().size() + patronOfInterest.getBorrowedMovies().size()
				+ patronOfInterest.getBorrowedBooks().size() >= 5) {
	...
	/**
	 * Places an item into the library contents as borrowable and removes from the
	 * patron's borrowed list
	 * 
	 * @param itemId
	 * @param patronId
	 * @return
	...
	public Item returnItem(int itemId, int patronId) throws IllegalArgumentException {
		...
		if (itemId < 0) {
			...
			// Remove item from patron's borrowed list by making a copy without the returned
			// item
	...
	public Item archiveItem(int itemID, int headLibrarianID) throws IllegalArgumentException {
		// To archive an item, need approval of head librarian. If there is a head
		// librarian ID associated with it, then it has been approved for the archives.
	 ...
	 * Only books, movies, and albums have some type of creator attribute (i.e.,
